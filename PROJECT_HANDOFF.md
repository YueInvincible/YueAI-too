...
- Desktop shell da co first permission center trong `Ops`:
  ...
  - recent risky actions/audit preview tu JSONL `audit.recent`.
- Audit log da co first read surface:
  - `AuditLog.recent(...)`;
...
- Luu y verify:
  ...
  - re-check ngay 2026-07-09 cho audit preview/permission event forwarding:
    - `PYTHONPATH=src python -m unittest tests.test_transport -v`
    - `PYTHONPATH=src;. PYTHONDONTWRITEBYTECODE=1 python -W error::ResourceWarning -m unittest discover -s tests -v`
    - `node --check desktop/src/app.js`
    - `node --check desktop/src/runtime.js`
    - `node --check desktop/src/state.js`
    - `node --test desktop/tests/protocol.test.js`
    - ket qua: transport tests pass 31/31, full Python unittest pass 143/143, JS syntax pass, protocol tests pass 17/17.
